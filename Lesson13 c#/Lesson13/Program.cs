namespace Lesson13
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\nTask 1\n");
            var task1 = debtors.Where(dLambda => dLambda.Email.EndsWith("rhyta.com") || dLambda.Email.EndsWith("dayrep.com")).ToList();
            task1.ForEach(dt => Console.WriteLine(dt));

            Console.WriteLine("\nTask 2\n");
            var task2 = debtors.Where(dLambda => DateTime.Now.Year - dLambda.BirthDay.Year >= 26 && DateTime.Now.Year - dLambda.BirthDay.Year <= 36).ToList();
            task2.ForEach(dt => Console.WriteLine(dt.BirthDay));

            Console.WriteLine("\nTask 3\n");
            var task3 = debtors.Where(dLambda => dLambda.Debt <= 5000).ToList();
            task3.ForEach(dt => Console.WriteLine(dt.Debt));

            Console.WriteLine("\nTask 4\n");
            var task4 = debtors.Where(dLambda => dLambda.FullName.Length >= 18 && dLambda.Phone.Count(ch => ch == '7') >= 2).ToList();
            task4.ForEach(dt => Console.WriteLine(dt.Phone + dt.FullName));
            Console.WriteLine("\nTask 5\n");

            var task5 = debtors.Where(dLambda => dLambda.BirthDay.Month == 12 || dLambda.BirthDay.Month == 1 || dLambda.BirthDay.Month == 2).ToList();
            task5.ForEach(dt => Console.WriteLine(dt.BirthDay));

            Console.WriteLine("\nTask 6\n");
            var task6 = debtors.Where(dLambda => dLambda.Debt > debtors.Average(dt => dt.Debt)).OrderByDescending(dt => dt.FullName.Split()[2]).OrderByDescending(dt => dt.Debt).ToList();
            task6.ForEach(dt => Console.WriteLine(dt));

            Console.WriteLine("\nTask 7\n");
            var task7 = debtors.Where(dLambda => !dLambda.Phone.Contains('8')).ToList();
            task7.ForEach(dt => Console.WriteLine(dt.FullName.Split()[2] + " " + dt.BirthDay + " " + dt.Debt));

            Console.WriteLine("\nTask 8\n");
            var task8 = debtors.Where(dLambda =>
            {
                for (int i = 0; i < dLambda.FullName.Length; i++)
                {
                    int sum = 0;
                    for (int j = 0; j < dLambda.FullName.Length; j++)
                    {
                        if (dLambda.FullName[i] == dLambda.FullName[j])
                        {
                            sum++;
                        }
                    }
                ...
                return false;
            }).OrderByDescending(dLambda => dLambda.FullName).ToList();
            task8.ForEach(dt => Console.WriteLine(dt));

            Console.WriteLine("\nTask 9\n");
            var task9 = debtors.Where(dLambda => dLambda.Debt == debtors.Max(dt => dt.Debt)).ToList();
            task9.ForEach(dt => Console.WriteLine(dt.BirthDay.Year));

            Console.WriteLine("\nTask 10\n");
            var task10 = debtors.OrderByDescending(dLambda => dLambda.Debt).ToList();
            for (int i = 0; i < 5; i++)
            {
            ...
            Console.WriteLine("\nTask 11\n");
            Console.WriteLine(debtors.Sum(dLambda => dLambda.Debt));

            Console.WriteLine("\nTask 12\n");
            var task12 = debtors.Where(dLambda => dLambda.BirthDay.Year <= 1945).ToList();
            task12.ForEach(dt => Console.WriteLine(dt));

            Console.WriteLine("\nTask 13\n");
            var task13 = debtors.Where(dLambda =>
            {
                bool tf = true;
                for (int i = 0; i < dLambda.Phone.Length; i++)
                {
                    int sum = 0;
                    for (int j = 0; j < dLambda.Phone.Length; j++)
                    {
                        if (dLambda.Phone[i] == dLambda.Phone[j])
                        {
                            sum++;
                        }
                    }
            ...
            task13.ForEach(dLambda => Console.WriteLine(dLambda.Debt));

            Console.WriteLine("\nTask 14\n");
            var task14 = debtors.Where(dLambda =>
            {
                DateTime date = DateTime.Now;
                while (date.Month != dLambda.BirthDay.Month)
                {
                    dLambda.Debt -= 500;
                    date = date.AddMonths(1);
                }
                dLambda.Debt -= 500;
                return dLambda.Debt <= 0 ? true : false;
            }).ToList();
            task14.ForEach(dLambda => Console.WriteLine(dLambda));

            Console.WriteLine("\nTask 15\n");
            var task15 = debtors.Where(dLambda => dLambda.FullName.ToLower().Contains("smile")).ToList();
            task15.ForEach(dt => Console.WriteLine(dt.FullName));
        }
    }
}